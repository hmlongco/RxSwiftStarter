...
public final class DismissibleSubject<ReturnType>: NSObject {
    ...
    public func onDismiss(perform action: DismissibleAction) -> Observable<ReturnType> {
        self.asObservable()
    ...
    ///
    /// - Returns: Void
    public func onDismiss(perform action: DismissibleAction, subscribe: @escaping DismissibleSubscription<ReturnType>) {
        self.asObservable()
            .subscribe({ [weak self] event in
    ...
    internal func checkObservedStatus() {
        if observed == false {
            #if swift(>=4.2)
            #warning("Dismissible was dismissed but was not observed.")
            #else
